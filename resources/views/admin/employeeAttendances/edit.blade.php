    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.employee-attendances.update", [$employeeAttendance->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('name') ? 'is-invalid' : '' }}" name="name_id" id="name_id">
                    @foreach($names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('name_id') ? old('name_id') : $employeeAttendance->name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $employeeAttendance->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('arrival') ? 'is-invalid' : '' }}" type="text" name="arrival" id="arrival" value="{{ old('arrival', $employeeAttendance->arrival) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('departure') ? 'is-invalid' : '' }}" type="text" name="departure" id="departure" value="{{ old('departure', $employeeAttendance->departure) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $employeeAttendance->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
