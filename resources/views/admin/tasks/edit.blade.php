...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.update", [$task->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="due_time">{{ trans('cruds.task.fields.due_time') }}</label>
                <input class="form-control timepicker {{ $errors->has('due_time') ? 'is-invalid' : '' }}" type="text" name="due_time" id="due_time" value="{{ old('due_time', $task->due_time) }}">
                @if($errors->has('due_time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('due_time') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.task.fields.due_time_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($task) && $task->image)
      var files = {!! json_encode($task->image) !!}
          for (var i in files) {
          var file = files[i]
