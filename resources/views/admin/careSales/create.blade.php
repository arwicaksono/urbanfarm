...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.careSale.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.care-sales.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('problem_sale') ? 'is-invalid' : '' }}" name="problem_sale_id" id="problem_sale_id">
                    @foreach($problem_sales as $id => $entry)
                        <option value="{{ $id }}" {{ old('problem_sale_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
