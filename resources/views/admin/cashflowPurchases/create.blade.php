...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cashflowPurchase.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cashflow-purchases.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_expense') ? 'is-invalid' : '' }}" name="is_expense" id="is_expense">
                    ...
                    @foreach(App\Models\CashflowPurchase::IS_EXPENSE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_expense', 'yes') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
