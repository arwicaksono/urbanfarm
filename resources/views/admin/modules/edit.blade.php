    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.modules.update", [$module->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $module->code) }}">
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="number" name="number" id="number" value="{{ old('number', $module->number) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('site_code') ? 'is-invalid' : '' }}" name="site_code_id" id="site_code_id">
                    @foreach($site_codes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_code_id') ? old('site_code_id') : $module->site_code->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('system') ? 'is-invalid' : '' }}" name="system_id" id="system_id">
                    @foreach($systems as $id => $entry)
                        <option value="{{ $id }}" {{ (old('system_id') ? old('system_id') : $module->system->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lighting') ? 'is-invalid' : '' }}" name="lighting_id" id="lighting_id">
                    @foreach($lightings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lighting_id') ? old('lighting_id') : $module->lighting->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reservoir') ? 'is-invalid' : '' }}" name="reservoir_id" id="reservoir_id">
                    @foreach($reservoirs as $id => $entry)
                        <option value="{{ $id }}" {{ (old('reservoir_id') ? old('reservoir_id') : $module->reservoir->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pump') ? 'is-invalid' : '' }}" name="pump_id" id="pump_id">
                    @foreach($pumps as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pump_id') ? old('pump_id') : $module->pump->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('pump'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mounting') ? 'is-invalid' : '' }}" name="mounting_id" id="mounting_id">
                    @foreach($mountings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('mounting_id') ? old('mounting_id') : $module->mounting->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('capacity') ? 'is-invalid' : '' }}" type="number" name="capacity" id="capacity" value="{{ old('capacity', $module->capacity) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id">
                    @foreach($units as $id => $entry)
                        <option value="{{ $id }}" {{ (old('unit_id') ? old('unit_id') : $module->unit->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('acitivities') ? 'is-invalid' : '' }}" name="acitivities[]" id="acitivities" multiple>
                    @foreach($acitivities as $id => $acitivity)
                        <option value="{{ $id }}" {{ (in_array($id, old('acitivities', [])) || $module->acitivities->contains($id)) ? 'selected' : '' }}>{{ $acitivity }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active">
                    ...
                    @foreach(App\Models\Module::IS_ACTIVE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_active', $module->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
