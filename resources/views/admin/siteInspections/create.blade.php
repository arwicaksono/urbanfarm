...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.siteInspection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.site-inspections.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('humidity') ? 'is-invalid' : '' }}" type="number" name="humidity" id="humidity" value="{{ old('humidity', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('weather') ? 'is-invalid' : '' }}" name="weather_id" id="weather_id">
                    @foreach($weather as $id => $entry)
                        <option value="{{ $id }}" {{ old('weather_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
