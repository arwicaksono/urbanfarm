...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.careModule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.care-modules.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('problem_mo') ? 'is-invalid' : '' }}" name="problem_mo_id" id="problem_mo_id">
                    @foreach($problem_mos as $id => $entry)
                        <option value="{{ $id }}" {{ old('problem_mo_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
