    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sales-channels.update", [$salesChannel->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $salesChannel->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('sales_type') ? 'is-invalid' : '' }}" name="sales_type" id="sales_type">
                    ...
                    @foreach(App\Models\SalesChannel::SALES_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('sales_type', $salesChannel->sales_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $salesChannel->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
