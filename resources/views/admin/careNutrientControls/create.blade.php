...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.careNutrientControl.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.care-nutrient-controls.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('problem_nc') ? 'is-invalid' : '' }}" name="problem_nc_id" id="problem_nc_id">
                    @foreach($problem_ncs as $id => $entry)
                        <option value="{{ $id }}" {{ old('problem_nc_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
