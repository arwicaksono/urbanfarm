...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.carePacking.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.care-packings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('problem_packing') ? 'is-invalid' : '' }}" name="problem_packing_id" id="problem_packing_id">
                    @foreach($problem_packings as $id => $entry)
                        <option value="{{ $id }}" {{ old('problem_packing_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
