...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.plotCode.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.plot-codes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('plant') ? 'is-invalid' : '' }}" type="text" name="plant" id="plant" value="{{ old('plant', '') }}">
                @if($errors->has('plant'))
                    <div class="invalid-feedback">
                        {{ $errors->first('plant') }}
                    </div>
