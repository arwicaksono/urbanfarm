...
class Packing extends Model implements HasMedia
{
    use SoftDeletes;
    ...
    public static function boot() {
        ...
        static::creating(function($model) {
            $model->number = Packing::whereYear('created_at', Carbon::now()->year)->where('team_id', $model->team_id)->max('number') + 1; // start from 1 every year
            $model->code = 'PKG' . str_pad($model->number, 5, '0', STR_PAD_LEFT)
            ;
        });
    }
}
