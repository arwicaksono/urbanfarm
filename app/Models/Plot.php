...
class Plot extends Model implements HasMedia
{
    ...
    public static function boot() {
        ...
        static::creating(function($model) {
            $model->number = Plot::whereYear('created_at', Carbon::now()->year)->where('team_id', $model->team_id)->max('number') + 1; // start from 1 every year
            $model->code = $model->plot_prefix->prefix . str_pad($model->number, 5, '0', STR_PAD_LEFT)
            ;
        });
    }
}
